...
class Whois{
	...
	public function info(){
		if($this->isValid()){
			$out = [];
			$whois = $this->whois();
			$start = strpos($whois, 'domain');
			$length = strpos($whois, 'TCI') - $start + 3;
			$whois_array = explode("\n", substr($whois, $start, $length));
			foreach($whois_array as $k => $row){
				$row_array = explode(': ', $row);
				if(count($row_array) == 2){
					$key = trim($row_array[0]);
					$value = trim($row_array[1]);
					if($key == 'domain'){
						$out[$key] = strtolower($value);
					}else if($key == 'nserver'){
						$out['nserver'][] = $value;
					}else if(in_array($key, ['created', 'paid-till', 'free-date'])){
						$date = explode("T", $value);
						$date = explode('-', $date[0]);
						$out[str_replace('-', '_', $key)] = mktime(0, 0, 0, $date[1], $date[2], $date[0]);
					}else{
						$out[$key] = $value;
					}
				}
			}
			$out['mxserver'] = $this->getMxServers();
			return $out;
		}else{
	...
	public function isValid(){
		if(isset($this->servers[$this->TLDs][0]) && strlen($this->servers[$this->TLDs][0]) > 6){
			$tmp_domain = strtolower($this->subDomain);
			if(preg_match("/^[a-z0-9\-]{3,}$/", $tmp_domain) && !preg_match("/^-|-$/", $tmp_domain) //&& !preg_match("/--/", $tmp_domain)
			){
				return true;
			}
		}
